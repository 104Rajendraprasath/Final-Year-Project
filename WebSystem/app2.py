...
# --- SALESFORCE CONFIGURATION (FILL THESE IN) ---
SF_USERNAME = ""
SF_PASSWORD = ""
SF_TOKEN = "" # Reset this in Salesforce Setup > My Personal Information > Reset Security Token
SF_CONSUMER_KEY = ""
SF_CONSUMER_SECRET = ""
...
try:
    ...
    # Logic to find the violence label ID automatically
...
def send_to_salesforce(camera_id, location, confidence):
    try:
        ...
        # Create the Record
        # NOTE: Ensure 'Security_Alert__c' exists in your Salesforce Object Manager
        sf.Security_Alert__c.create({
            'Camera_ID__c': camera_id,
            'Location__c': location,
            'Confidence__c': confidence * 100, # Convert 0.95 to 95
            'Status__c': 'New',
            'Incident_Time__c': datetime.now().isoformat()
        })
        print(f"[SALESFORCE] Alert successfully sent for {camera_id}")
        return True
    except Exception as e:
...
def process_feed():
    ...
    if label == violence_label_name:
        ...
        # --- TRIGGER SALESFORCE ALERT ---
        print("[INFO] Violence confirmed. Triggering Salesforce Alert...")
        # We run this synchronously for now. In production, use a background thread/Celery.
